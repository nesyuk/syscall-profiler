...
static bool (*test_mem_ops[5])(void **, unsigned int *) = {
    test_malloc, test_free, test_calloc, test_realloc, test_free};
static bool (*test_fd_ops[4])(int *, size_t *) = {test_open, test_read,
                                                  test_write, test_close};
...
void test_rand(int num_of_syscalls) {
  ...
  for (int i = 0; i < num_of_syscalls; i++) {
    ...
    while (!op_allowed) {
      ...
      if (random_op_idx < 5) {
        op_allowed = (*test_mem_ops[random_op_idx])(allocated_memory_list,
                                                    &allocated_memory_count);
      } else {
        op_allowed = (*test_fd_ops[random_op_idx - 5])(opened_files_list,
                                                       &opened_files_count);
      }
    }
  }
