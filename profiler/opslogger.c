...
static int file_flags[] = {
    O_APPEND,    O_ASYNC,    O_CLOEXEC, O_CREAT,       __O_TMPFILE, __O_DIRECT,
    O_DIRECTORY, O_DSYNC,    O_EXCL,    __O_LARGEFILE, __O_NOATIME, O_NOCTTY,
    O_NOFOLLOW,  O_NONBLOCK, __O_PATH,  O_SYNC,        O_TRUNC,     O_RDWR,
    O_RDONLY,    O_WRONLY,   O_EXCL};

static char *file_flags_names[] = {
    "O_APPEND",  "O_ASYNC",     "O_CLOEXEC",  "O_CREAT",    "O_TMPFILE",
    "O_DIRECT",  "O_DIRECTORY", "O_DSYNC",    "O_EXCL",     "O_LARGEFILE",
    "O_NOATIME", "O_NOCTTY",    "O_NOFOLLOW", "O_NONBLOCK", "O_PATH",
    "O_SYNC",    "O_TRUNC",     "O_RDWR",     "O_RDONLY",   "O_WRONLY",
    "O_EXCL"};
/*
 * Log syscall event
 */
void __log_syscall(const char *syscall, const char *args) {
  ...
  sprintf(buf, "%lld%10s %s%*s %s%*s\n", timestamp_ms, "", syscall,
          (int)(max_space - strlen(syscall)), "", args,
          (int)(max_space - strlen(args)), "");
  log_msg(buf);
}
...
void log_open(const char *pathname, int flags, mode_t mode) {
  ...
  sprintf(args, "pathname: %s, flags: %s(%d), mode: %d", pathname, flag_args,
          flags, mode);
