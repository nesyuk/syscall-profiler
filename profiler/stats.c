...
static unsigned long long stats_mem_overhead = 0;
...
static unsigned long long stats_fd_overhead = 0;
...
void __append_mem() {
  if (!stats_mem_head) {
    ...
    if (log_overhead_calc())
      stats_mem_overhead =
          sizeof(struct stat_mem_alloc) + sizeof(unsigned long long) * 3;
  }
...
void __append_fd() {
  if (!stats_fd_head) {
    ...
    if (log_overhead_calc())
      stats_fd_overhead =
          sizeof(struct stat_fd) + sizeof(unsigned long long) * 3;
  }
...
void __log_alloc() {
  struct stat_mem_alloc *cur = stats_mem_head;
  sprintf(buf,
          "Total allocated memory: %lld bytes (~%lldKB)\nTotal freed memory: "
          "%lld bytes (~%lldKB)\n",
          alloc_count, alloc_count / bytesToKb, free_count,
          free_count / bytesToKb);
  log_stats(buf);
  if (log_overhead_calc()) {
    sprintf(buf, "Total overhead: %lld bytes (~%lldKB)\n", stats_mem_overhead,
            stats_mem_overhead / bytesToKb);
    log_stats(buf);
  }
  ...
  sprintf(
      buf,
      "| ptr:%10s | malloc (bytes):%5s | calloc (nmemb x bytes):%20s | realloc "
      "(bytes):%4s | last size (bytes):%2s | total free (bytes):%1s |\n",
      "", "", "", "", "", "");
  log_stats(buf);
  while (cur != NULL) {
    if ((cur->malloc_size + (cur->calloc_size * cur->calloc_nmemb) +
         cur->realloc_size) != cur->free_size) {
      sprintf(buf, "| %p | %20ld | %20ld x %20ld | %20ld | %20ld | %20ld |\n",
              cur->ptr, cur->malloc_size, cur->calloc_size, cur->calloc_nmemb,
              cur->realloc_size, cur->last_size, cur->free_size);
      log_stats(buf);
    }
    cur = cur->next;
  }
...
void __log_fd() {
  ...
  if (log_overhead_calc()) {
    sprintf(buf, "Total overhead: %lld bytes (~%lldKB)\n", stats_fd_overhead,
            stats_fd_overhead / bytesToKb);
    log_stats(buf);
  }
  sprintf(buf, "\nFile operations info per file descriptor:\n\n");
  log_stats(buf);
  sprintf(buf,
          "| file descriptor #: %2s| open (times):%7s | read (bytes):%7s | "
          "write (bytes):%6s | close (times):%6s |\n",
          "", "", "", "", "");
  log_stats(buf);
  while (cur != NULL) {
    if (cur->open != cur->close) {
      sprintf(buf, "| %20d | %20ld | %20lld | %20lld | %20ld |\n", cur->fd,
              cur->open, cur->read, cur->write, cur->close);
      log_stats(buf);
    }
    cur = cur->next;
  }
